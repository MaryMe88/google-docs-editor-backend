...
/* ============================================================================
 * SECTION: UX CONFIG (человеческие описания режимов)
 *
 * Этот слой НЕ трогает контракт с backend. Он только объясняет,
 * что делает режим и когда его выбирать.
 * ============================================================================
 */

const MODE_UX_CONFIG = {
  basic_edit: {
    title: 'Базовая правка',
    summary: 'Аккуратно подчистит формулировки, орфографию и стилистику без сильного вмешательства в авторский голос.',
    when_to_use: 'Когда текст в целом нормальный, но хочется, чтобы он звучал чуть чище и профессиональнее.',
    avoid_when: 'Когда нужно радикально упростить, сократить или перестроить текст.',
    risk_level: 'низкий'
  },

  logic_edit: {
    title: 'Проверка логики',
    summary: 'Ищет логические разрывы, противоречия и слабые места в аргументации.',
    when_to_use: 'Когда мысль важная, но вы сомневаетесь в стройности аргументации и переходов.',
    avoid_when: 'Когда задача только стилистическая: заменить слова, но не трогать смысл.',
    risk_level: 'средний'
  },

  balanced_edit: {
    title: 'Взвешенная правка',
    summary: 'Комбинирует стилистическую и логическую правку, стараясь сохранить авторскую интонацию.',
    when_to_use: 'Когда текст нужно заметно улучшить по качеству, но без «переписывания с нуля».',
    avoid_when: 'Когда нужно либо совсем бережное вмешательство, либо наоборот — жёсткая чистка и сокращение.',
    risk_level: 'средний'
  },

  readerfirst: {
    title: 'Фокус на читателе',
    summary: 'Перестраивает текст так, чтобы читателю было проще понять главное и не потеряться.',
    when_to_use: 'Когда текст формально корректный, но тяжёлый для восприятия и хочется большего фокуса на пользе.',
    avoid_when: 'Когда нужен максимально буквальный пересказ без изменений акцентов.',
    risk_level: 'средний'
  },

  cutnoise: {
    title: 'Убрать мусор',
    summary: 'Сокращает лишние слова, повторы и воду, оставляя только важное.',
    when_to_use: 'Когда текст разросся, много повторов и общих слов, а хочется компактности.',
    avoid_when: 'Когда важна длина текста (например, нужен объём для макета или SEO).',
    risk_level: 'средний'
  },

  makeclear: {
    title: 'Упростить предложения',
    summary: 'Разбирает сложные конструкции на более простые и понятные фразы.',
    when_to_use: 'Когда предложения слишком длинные, с кучей запятых и вложенных мыслей.',
    avoid_when: 'Когда важен сложный, «литературный» синтаксис и длинные периоды.',
    risk_level: 'низкий'
  },

  restructure: {
    title: 'Перестроить структуру',
    summary: 'Меняет порядок абзацев и блоков, чтобы текст легче читался и лучше раскрывал тему.',
    when_to_use: 'Когда кажется, что всё сказано, но «не с того конца» или в странной последовательности.',
    avoid_when: 'Когда структура уже согласована (например, жёсткий формат заявки, ТЗ или юридический текст).',
    risk_level: 'повышенный'
  },

  deai: {
    title: 'Убрать признаки ИИ',
    summary: 'Ставит целью сделать текст менее «типично-ИИшным» и более живым, с отчётом об изменениях.',
    when_to_use: 'Когда текст похож на стандартный ИИ-ответ и нужно приблизить его к живой человеческой речи.',
    avoid_when: 'Когда важна строгая формальность и однородный корпоративный стиль.',
    risk_level: 'повышенный'
  }
};

/**
 * Возвращает UX-описание режима по modeId или null, если его нет.
 * @param {string} modeId
 * @returns {object|null}
 */
function getModeUx_(modeId) {
  if (!modeId || typeof modeId !== 'string') {
    return null;
  }
  const key = modeId.trim();
  return Object.prototype.hasOwnProperty.call(MODE_UX_CONFIG, key)
    ? MODE_UX_CONFIG[key]
    : null;
}
...
/**
 * Собирает человекочитаемую подпись для режима: название + краткое описание.
 * Если UX-конфиг не найден, использует menu из MODE_CONFIG.
 * @param {string} modeId
 * @returns {string}
 */
function getModeLabel_(modeId) {
  const mode = MODE_CONFIG[modeId];
  const ux = getModeUx_(modeId);

  if (!mode && !ux) {
    return modeId;
  }

  const title = (ux && ux.title) || (mode && mode.menu) || modeId;
  const summary = ux && ux.summary ? ux.summary : '';
...
function onOpen(e) {
  ...
  // Служебные пункты: откат, подсказка по режимам и настройка подключения.
  rootMenu.addSeparator();
  rootMenu.addItem('↩ Отменить последнюю правку', 'undoLastEdit');
  rootMenu.addItem('❓ Как выбрать режим', 'showModeHelp');
...
/* ============================================================================
 * SECTION: UX HELP (подсказки по выбору режимов)
 * ============================================================================
 */

/**
 * Показывает справку по основным режимам правки и чистки.
 * Запускается из меню «LLM редактор → ❓ Как выбрать режим».
 */
function showModeHelp() {
  ...
  lines.push('Как выбрать режим правки\n');
  lines.push(
    'Ниже — короткие подсказки по основным режимам. ' +
    'Выберите тот, который лучше всего совпадает с вашей задачей.\n'
  );
  ...
  modeOrder.forEach((modeId) => {
    ...
    const title = (ux && ux.title) || (mode && mode.menu) || modeId;
    const summary = ux && ux.summary ? ux.summary : '';
    const whenToUse = ux && ux.when_to_use ? ux.when_to_use : '';
    const avoidWhen = ux && ux.avoid_when ? ux.avoid_when : '';
    const risk = ux && ux.risk_level ? ux.risk_level : '';
    ...
    if (whenToUse) {
      lines.push('  Когда использовать: ' + whenToUse);
    }
    if (avoidWhen) {
      lines.push('  Осторожно: ' + avoidWhen);
    }
    if (risk) {
      lines.push('  Уровень вмешательства: ' + risk);
    }

    lines.push(''); // пустая строка между режимами
  });

  ui.alert('Подсказка по режимам', lines.join('\n'), ui.ButtonSet.OK);
}
