...
Сохраняет расширенную логику отбора правил, валидацию KB,
нормализацию тегов и диагностические селекторы.
"""
...
class StructuralEntry(TypedDict, total=False):
    """Структурная запись: шаблон, фреймворк, композиционный принцип."""
...
CANONICAL_TAGS: Dict[str, Dict[str, Any]] = {
    "domains": {
        ...
        "fiction": {
            "primary": ["fiction"],
            "expanded": ["narrative", "creative"],
        },
        "basic_edit": {
            "primary": ["editing"],
            "expanded": ["clarity", "grammar", "style"],
        },
        "logic_edit": {
            "primary": ["logic"],
    ...
    "intents": {
        "analytical": {
            "primary": ["logic", "clarity"],
            "expanded": ["coherence", "precision", "argumentation"],
        },
        "storytelling": {
        ...
        "marketingpush": {
            "primary": ["marketing"],
            "expanded": ["persuasion", "cta", "conversion"],
        },
        "noragal": {
    ...
    "overlays": {
        ...
        "recommendations": {
            ...
            "expanded": ["advice", "improvements"],
        },
        "finalcheck": {
            "primary": ["finalcheck"],
            "expanded": ["proofread", "cleanup"],
        },
        "marketingpush": {
            "primary": ["marketing"],
...
KNOWN_INTENTS: Set[str] = {normalize_tag(key) for key in CANONICAL_TAGS["intents"].keys()}
KNOWN_OVERLAYS: Set[str] = {normalize_tag(key) for key in CANONICAL_TAGS["overlays"].keys()}
...
def load_intent_config(intent: Optional[str], base_path: Path = Path("config")) -> Optional[IntentConfig]:
    if intent is None or normalize_tag(intent) == "neutral":
        return None
    data = load_json_file(base_path / "intents" / f"{normalize_tag(intent)}.json")
    return IntentConfig(name=data["name"], instructions=data["instructions"])


def load_overlay_config(overlay: str, base_path: Path = Path("config")) -> OverlayConfig:
    data = load_json_file(base_path / "overlays" / f"{normalize_tag(overlay)}.json")
    return OverlayConfig(name=data["name"], instructions=data["instructions"])
...
def _flatten_examples_block(items: List[Dict[str, Any]], category: str = "") -> List[FlatEntry]:
    flat: List[FlatEntry] = []
    for item in items:
        ...
        if "examples" in item:
            ...
            for example in examples:
                ...
                if "tags" not in entry:
                    entry["tags"] = item.get("tags", ["style"])
            ...
            flat.append(
                {
                    ...
                    "example_wrong": example.get("wrong", ""),
                    "example_correct": example.get("correct", ""),
                    "example_explanation": example.get("explanation", ""),
                    "tags": normalize_tags(list(block_tags) + list(tech.get("tags", [])))
                    if block_tags or tech.get("tags")
                    else normalize_tags(["editing", "noragal"]),
...
def _score_rule_entry(
    ...
    if overlap > 0:
        score += 1
    if expanded_tags:
        score += len(tag_set & expanded_tags) * 2
    return score, -idx
...
def _score_structural_entry(
    ...
    if isinstance(when_to_use, str) and when_to_use.strip():
        patterns.append(when_to_use.strip())
    ...
    for pattern in unique_patterns:
        if _contains_pattern(normalized_text, pattern):
            score += 500 if pattern == str(entry.get("name", "")).strip() else 200
            break
...
def _log_selection_debug(
    ...
    for item in scored[:5]:
        entry = item[2]
        score_value = item[0]
        name = str(entry.get("name", entry.get("wrong", "?")))[:30]
        if score_value >= 1000:
            reason = "text_match"
...
def build_nkrj_norms_lines(kb: KnowledgeBase, limit_sources: int = 4) -> List[str]:
    ...
    if avg is not None:
        lines.append(
            f" • Ориентир по длине предложения: в среднем около {avg:.2f} слов; держи фразы преимущественно короткими и средними."
        )
    if short_share is not None and medium_share is not None and long_share is not None:
        lines.append(
            f" • Распределение длины предложений: короткие ≈ {short_share:.1%}, средние ≈ {medium_share:.1%}, длинные ≈ {long_share:.1%}."
        )
    if variation is not None:
        lines.append(
            f" • Коэффициент вариативности длины предложений — около {variation:.2f}; избегай монотонного ритма."
        )

    flat_paragraph = _safe_float(aggregate.get("norm_flat_paragraph_share"))
    if flat_paragraph is not None:
        lines.append(
            f" • Плоские абзацы почти не встречаются: норма flat paragraph share ≈ {flat_paragraph:.2%}."
        )
    ...
    if deepr_rate is not None:
        lines.append(f" • Глубокие шаблонные клише почти отсутствуют: норма ≈ {deepr_rate:.2f} на 100 строк.")
    ...
    if plasticity_live is not None and plasticity_grey is not None:
        lines.append(
            f" • Индекс пластичности: до {plasticity_live:.1f} — живой текст, около {plasticity_grey:.1f} и выше — риск искусственности."
        )

    sentence_variation_min = _safe_float(thresholds.get("sentence_variation_coeff_min"))
    if sentence_variation_min is not None:
        lines.append(
            f" • Нижняя граница вариативности длины предложений: ≈ {sentence_variation_min:.2f}; не выравнивай ритм до механической ровности."
        )

    short_sentence_share_min = _safe_float(thresholds.get("short_sentence_share_min"))
    if short_sentence_share_min is not None:
        lines.append(
            f" • Доля коротких предложений обычно не ниже {short_sentence_share_min:.1%}; длинные периоды должны разбавляться короткими фразами."
        )

    flat_alert = _safe_float(thresholds.get("flat_paragraph_share_alert"))
    if flat_alert is not None:
        lines.append(
            f" • Сигнал риска по однотипным абзацам начинается около {flat_alert:.0%}; следи за внутренней динамикой абзаца."
        )

    passive_alert = _safe_float(thresholds.get("passive_rate_alert"))
    if passive_alert is not None:
        lines.append(
            f" • Повышенный риск перегруза пассивом начинается примерно от {passive_alert:.1f} на 100 строк."
        )
    ...
    if marker_examples:
        ...
        lines.append(
            " • Пластиковые маркеры, за которыми стоит следить: " + ", ".join(unique_markers[:8]) + "."
        )
    return lines


def _has_mode(intent: Optional[str], overlays: Sequence[str], aliases: Iterable[str]) -> bool:
...
def _get_canonical_tags_for_category(category: str, value: str) -> List[str]:
    ...
    data = CANONICAL_TAGS.get(category, {}).get(normalized_value)
    if isinstance(data, dict):
        return normalize_tags(data.get("primary", []) + data.get("expanded", []))
    if isinstance(data, list):
        return normalize_tags(data)
    return normalize_tags([normalized_value])
...
class PromptBuilder:
    ...
    def _get_domain_config(self, domain: str) -> DomainConfig:
        normalized_domain = normalize_tag(domain)
        if normalized_domain not in self._domain_cache:
            self._domain_cache[normalized_domain] = load_domain_config(normalized_domain, self.config_path)
    ...
    def _get_overlay_config(self, overlay: str) -> OverlayConfig:
        ...
        if normalized_overlay not in self._overlay_cache:
            self._overlay_cache[normalized_overlay] = load_overlay_config(normalized_overlay, self.config_path)
        return self._overlay_cache[normalized_overlay]

    def _get_intent_config(self, intent: Optional[str]) -> Optional[IntentConfig]:
        if intent is None or normalize_tag(intent) == "neutral":
            return None
        normalized_intent = normalize_tag(intent)
        if normalized_intent not in self._intent_cache:
            config = load_intent_config(normalized_intent, self.config_path)
            if config is None:
                return None
            self._intent_cache[normalized_intent] = config
        return self._intent_cache[normalized_intent]
    ...
    def _get_available_intents(self) -> Set[str]:
        if self._available_intents_cache is None:
            ...
            if not intents_dir.exists():
                ...
                self._available_intents_cache = {normalize_tag(path.stem) for path in intents_dir.glob("*.json")}
    ...
    def _get_available_overlays(self) -> Set[str]:
        if self._available_overlays_cache is None:
            ...
            if not overlays_dir.exists():
                ...
                self._available_overlays_cache = {normalize_tag(path.stem) for path in overlays_dir.glob("*.json")}
    ...
    def build(
        ...
        normalized_domain = normalize_tag(domain)
        normalized_intent = normalize_tag(intent) if intent else None
        normalized_overlays = normalize_tags(overlays)

        parts: List[str] = []
        parts.append(self._build_core_block())
        parts.append(self._build_domain_block(normalized_domain))
        if normalized_intent:
            intent_block = self._build_intent_block(normalized_intent)
            if intent_block:
                parts.append(intent_block)
        parts.append(self._build_audience_block(audience))
        if normalized_overlays:
            parts.append(self._build_overlays_block(normalized_overlays))
    ...
    def _build_domain_block(self, domain: str) -> str:
        ...
        return f"Домен: {domain_config.system_rules}\nТон: {domain_config.tone}"

    def _build_intent_block(self, intent: str) -> str:
        intent_config = self._get_intent_config(intent)
        if intent_config is None:
            return ""
        instructions = "\n".join(f"- {instruction}" for instruction in intent_config.instructions)
        return f"Цель обработки: {intent_config.name}\n\nТребования:\n{instructions}"
    ...
    def _build_overlays_block(self, overlays: Sequence[str]) -> str:
        ...
        for overlay in overlays:
            config = self._get_overlay_config(overlay)
    ...
    def _resolve_prompt_features(
        ...
        if intent is not None:
            ...
            normalized_intent = normalize_tag(intent)
            if normalized_intent not in KNOWN_INTENTS and normalized_intent not in available_intents:
        ...
        for overlay in overlays:
            ...
            normalized_overlay = normalize_tag(overlay)
            if normalized_overlay not in KNOWN_OVERLAYS and normalized_overlay not in available_overlays:
        ...
        marketing_requested = _has_mode(intent, overlays, {"marketingpush", "marketing", "sales"})

        return {
            "tags": list(primary_set),
            "expanded_tags": list(expanded_set),
            "storytelling_enabled": domain_config.allow_storytelling and storytelling_requested,
            "marketing_enabled": domain_config.allow_marketing and (domain == "marketing" or marketing_requested),
        }
    ...
    def _build_grammar_style_logic_block(
        ...
        grammar_lines = [
            f" • {item.get('wrong', '')} → {item.get('correct', '').strip()} ({item.get('rule', '').strip()})"
            for item in grammar_sample
            if item.get("wrong") and item.get("correct")
        ] or [" • (нет примеров в базе)"]

        style_lines = [
            f" • {item.get('wrong', '')} → {item.get('correct', '').strip()} ({item.get('rule', '').strip()})"
            for item in style_sample
            if item.get("wrong")
        ] or [" • (нет примеров в базе)"]

        logic_lines = [
            f" • {item.get('name', item.get('wrong', 'Проблема'))}: {item.get('rule', item.get('description', '')).strip()}"
    ...
    def _build_composition_cohesion_errors_block(
        ...
        composition_lines = [
            f" • {entry.get('name', '')}: {entry.get('rule', entry.get('description', '')).strip()}"
    ...
    def _build_nkrj_block(self, kb: KnowledgeBase) -> str:
        norms = build_nkrj_norms_lines(kb)
        if not norms:
            return ""
        return (
    ...
    def _build_marketing_block(
        ...
        for template in templates_sample:
            ...
            if name and section_names:
                lines.append(f" • {name}: " + ", ".join(section_names))
        if not lines:
            return ""
    ...
    def _build_rhetoric_editorial_glossary_block(
        ...
        if kb.editorial_techniques:
            ...
            for tech in editorial_sample:
                ...
                if wrong or correct:
                    example_pair = f"Пример: {wrong} → {correct}"
                    if explanation:
                        example_pair += f" ({explanation.strip()})"
                    line += f". {example_pair}"
        ...
        if kb.domain_glossary:
            ...
            for glossary_domain, glossary_terms in kb.domain_glossary.items():
            ...
            if len(relevant_terms) < self.glossary_limit:
                domains_to_check = [domain] + [key for key in kb.domain_glossary.keys() if key != domain]
                for glossary_domain in domains_to_check:
                    glossary_terms = kb.domain_glossary.get(glossary_domain)
                    if not isinstance(glossary_terms, dict):
                        continue
                    if glossary_domain == domain or normalize_tag(glossary_domain) in wanted_tags_set:
                        for term, definition in glossary_terms.items():
            ...
            if relevant_terms:
                items = list(relevant_terms.items())[: self.glossary_limit]
                term_lines = [f" • {key}: {value}" for key, value in items]
    ...
    def _build_stop_words_block(self, kb: KnowledgeBase, primary_tags: List[str]) -> str:
        ...
        ordered_categories = priority_categories + sorted(other_categories, key=lambda item: item[0].lower())
        lines: List[str] = []
    ...
    def _build_knowledge_block(
        ...
        domain_config = self._get_domain_config(domain)
        features = self._resolve_prompt_features(domain_config, domain, intent, overlays)
        tags = features["tags"]
    ...
    @staticmethod
    def _build_text_block(text: str) -> str:
...
def validate_entry_tags(entry: Dict[str, Any], entry_name: str, index: int, strict: bool = True) -> None:
    ...
    for tag_index, tag in enumerate(tags):
        if not isinstance(tag, str):
            raise ValueError(
                f"{entry_name}[{index}].tags[{tag_index}] must be str, got {type(tag)}"
            )
...
def _validate_stop_words_structure(stop_words: Any) -> None:
    ...
    for category, words in stop_words.items():
        ...
        for index, word in enumerate(words):
            if not isinstance(word, str):
                raise ValueError(
                    f"stop_words['{category}'][{index}] must be str, got {type(word)}"
                )
...
def _validate_rule_entries(entries: List[Dict[str, Any]], name: str, sample_size: int = 5) -> None:
    ...
    for index, entry in enumerate(entries[:sample_size]):
        if not isinstance(entry, dict):
            raise ValueError(f"{name}[{index}] must be a dict, got {type(entry)}")
        for key in ("wrong", "correct", "rule", "description", "name", "category"):
            if key in entry and not isinstance(entry[key], str):
                raise ValueError(f"{name}[{index}].{key} must be str, got {type(entry[key])}")
        validate_entry_tags(entry, name, index, strict=KB_TAGS_STRICT_VALIDATION)
        if not any(isinstance(entry.get(key), str) and entry.get(key, "").strip() for key in ("wrong", "rule", "description", "name")):
            raise ValueError(
                f"{name}[{index}] must contain non-empty 'wrong', 'rule', 'description', or 'name'"
            )
...
def _validate_structural_entries(entries: List[Dict[str, Any]], name: str, sample_size: int = 5) -> None:
    ...
    for index, entry in enumerate(entries[:sample_size]):
        ...
        if "name" not in entry or not isinstance(entry["name"], str) or not entry["name"].strip():
            raise ValueError(f"{name}[{index}] must have a non-empty 'name'")
        for container_key in ("steps", "sections"):
            container = entry.get(container_key)
            if container is not None and not isinstance(container, list):
                raise ValueError(f"{name}[{index}].{container_key} must be a list if present")
        validate_entry_tags(entry, name, index, strict=KB_TAGS_STRICT_VALIDATION)
...
def _validate_logic_entries(entries: List[Dict[str, Any]], name: str, sample_size: int = 5) -> None:
    ...
    for index, entry in enumerate(entries[:sample_size]):
        if not isinstance(entry, dict):
            raise ValueError(f"{name}[{index}] must be a dict, got {type(entry)}")
        for key in ("name", "wrong", "rule", "description", "category"):
            if key in entry and not isinstance(entry[key], str):
                raise ValueError(f"{name}[{index}].{key} must be str, got {type(entry[key])}")
        validate_entry_tags(entry, name, index, strict=KB_TAGS_STRICT_VALIDATION)
        if not any(isinstance(entry.get(key), str) and entry.get(key, "").strip() for key in ("name", "wrong", "rule", "description")):
            raise ValueError(
                f"{name}[{index}] must contain non-empty 'name', 'wrong', 'rule', or 'description'"
            )
...
def validate_configs_and_kb(config_path: Path = Path("config"), kb_path: Path = Path("knowledge_base")) -> None:
    try:
    ...
    try:
        ...
        if domain_files:
            domain_config = load_domain_config(domain_files[0].stem, config_path)
            if not domain_config.system_rules:
    ...
    try:
        ...
        if intent_files:
            intent_config = load_intent_config(intent_files[0].stem, config_path)
            if intent_config is not None and not intent_config.instructions:
    ...
    try:
        ...
        if overlay_files:
            overlay_config = load_overlay_config(overlay_files[0].stem, config_path)
            if not overlay_config.instructions:
    ...
    try:
        output_format = load_output_format("text_only", config_path)
...
def _self_test_tag_normalization() -> None:
    ...
    assert normalize_tag("final_check") == "finalcheck"
    ...
    assert result == ["antiai", "humanize"], f"dedup + normalize failed: {result}"
    ...
    primary_marketing = _get_primary_tags_for_category("intents", "marketingpush")
    assert "marketing" in primary_marketing
    ...
    assert "finalcheck" in KNOWN_TAGS

    validate_entry_tags({"tags": ["antiai", "humanize"]}, entry_name="test", index=0, strict=True)
