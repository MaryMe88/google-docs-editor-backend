...
# Допустимые значения edit_level для домена (см. DomainConfig.edit_level).
ALLOWED_EDIT_LEVELS: frozenset = frozenset({"light", "processing", "remake", "adaptive_remake"})
...
def load_domain_config(domain: str, base_path: Path = Path("config")) -> DomainConfig:
    ...
    if not isinstance(raw_edit_level, str) or raw_edit_level not in ALLOWED_EDIT_LEVELS:
        logger.warning(
            ...
            raw_edit_level, data.get("name", normalized_domain), sorted(ALLOWED_EDIT_LEVELS),
        )
...
class PromptBuilder:
    ...
    def _build_edit_level_block(self, domain_config: DomainConfig) -> str:
        """Явная инструкция о допустимой глубине структурного вмешательства.

        Ранее edit_level существовал только как декоративная заметка в JSON
        и не влиял на промпт. Теперь это реальный параметр: значение читается
        из domain_config.edit_level и превращается в конкретную инструкцию.
        """
