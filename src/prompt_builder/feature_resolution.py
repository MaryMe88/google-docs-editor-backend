...
def _prepare_resolution_context(
    ...
    return result, effective_intent, effective_overlays, tags, suppressed_layers, warnings


def _apply_domain_incompatibility(
    result: FeatureResolutionResult,
    domain: str,
    ...
    return effective_intent, effective_overlays, tags
...
def _handle_explicit_suppress_in_conflict(
    ...
    ov: str,
    conflict: str,
    cfg_ov: OverlayConfig,
    cfg_conflict: OverlayConfig,
    effective_overlays: list[str],
    tags: list[str],
    suppressed_layers: list[str],
    warnings: list[str],
    ...
    if any(_normalize_overlay_ref(s) == conflict for s in cfg_ov.suppresses):
        ...
        suppressed_layers.append(
            f"overlay '{conflict}' suppressed by overlay '{ov}' (explicit suppress)"
        )
    ...
    if any(_normalize_overlay_ref(s) == ov for s in cfg_conflict.suppresses):
        ...
        suppressed_layers.append(
            f"overlay '{ov}' suppressed by overlay '{conflict}' (explicit suppress)"
        )
...
def resolve_prompt_features(
    ...
    _activate_features_from_tags(
        result, domain_config, effective_intent, effective_overlays, domain
    )
