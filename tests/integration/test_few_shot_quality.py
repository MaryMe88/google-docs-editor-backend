...
def has_grammar_error_after_soglasno(text: str) -> bool:
    ...
    """
    pattern = r'согласно\s+[а-яё]+\s*[а-яё]*\b[^у]\b'  # упрощённо
    # Более точная проверка: после "согласно" идёт слово в родительном падеже
    # (оканчивается на -а, -я, кроме исключений). Для теста достаточно простого поиска.
...
def test_no_false_positive_from_few_shot(client: TestClient) -> None:
    ...
    # Проверяем, что модель не добавила ошибочную форму 'приказа'
    assert "приказа" not in result.lower(), f"Появилась ошибка 'приказа': {result}"
    # Модель может перефразировать, но не должна ломать грамматику
    # Проверяем, что после 'согласно' нет родительного падежа
    assert not has_grammar_error_after_soglasno(result), \
        f"Грамматическая ошибка после 'согласно' в ответе: {result}"
    # Дополнительно: если модель убрала 'согласно', это допустимо
    # Главное – нет явной ошибки
