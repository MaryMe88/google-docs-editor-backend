...
class TestPromptBuilderCache:
    ...
    def test_get_overlay_config_caches(self, builder):
        with patch(
            "src.prompt_builder.builder.load_overlay_config", wraps=load_overlay_config
        ) as mock_load:
    ...
    def test_get_output_format_caches(self, builder):
        with patch(
            "src.prompt_builder.builder.load_output_format", wraps=load_output_format
        ) as mock_load:
    ...
    def test_reload_configs_clears_caches(self, builder):
        ...
        with (
            patch(
                "src.prompt_builder.builder.load_domain_config",
                wraps=load_domain_config,
            ) as mock_load_domain,
            patch(
                "src.prompt_builder.builder.load_intent_config",
                wraps=load_intent_config,
            ) as mock_load_intent,
            patch(
                "src.prompt_builder.builder.load_overlay_config",
                wraps=load_overlay_config,
            ) as mock_load_overlay,
            patch(
                "src.prompt_builder.builder.load_output_format",
                wraps=load_output_format,
            ) as mock_load_output,
        ):
            builder.get_domain_config("blog")
            builder.get_intent_config("storytelling")
    ...
    def test_get_knowledge_base_caches(self, builder):
        ...
        with patch(
            "src.prompt_builder.builder.load_knowledge_base", wraps=load_knowledge_base
        ) as mock_load_kb:
